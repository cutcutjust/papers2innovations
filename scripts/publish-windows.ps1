...
$Manifest = [ordered]@{
  version = $Version
  notes = "v0.1.35 replaces fragile full-page PDF vision requests with PyMuPDF region reconstruction. Two-column body text, large table chunks, numbered formulas, figures, and reference columns are processed independently with task-specific timeouts and resumable caches. Failed or unknown-cost regions are never retried automatically, broken local fallback text is no longer published, and a failed reparse cannot overwrite the last readable revision. Activity now reports explicit stage states and real page/region totals instead of treating 100% progress as success. DSSR regression covers 5/5 pages, two ten-column tables, formulas (1)-(13), Figure 1/2, and 33 ordered references. Existing libraries, models, prompts, Stronghold credentials, translations, and conversations remain compatible across the upgrade."
