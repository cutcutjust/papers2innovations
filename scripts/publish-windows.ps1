...
$Manifest = [ordered]@{
  version = $Version
  notes = "Reader translations now recover every sentence anchor after whitespace-only Markdown reformatting, so existing translations regain their independent underlines without another model call. Translation requests expose reasoning-only progress, enforce complete structured output, split long passages safely, and show continue-waiting or stop controls after 90 seconds without retrying or duplicating charges. White, warm paper, soft green, and dark reading palettes now style the complete Reader workspace while leaving the global application navigation unchanged. Existing libraries, settings, prompts, model assignments, conversations, translations, and Stronghold credentials remain compatible across the upgrade."
