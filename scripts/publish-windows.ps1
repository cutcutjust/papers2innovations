...
$Manifest = [ordered]@{
  version = $Version
  notes = "AI requests now provide immediate, truthful feedback from request preparation through connection, first token, generation, saving, and completion, with elapsed time and token usage. Technical terminology is highlighted consistently in both English source text and Chinese translations, while ordinary academic wording is filtered out. The larger terminology panel presents domain-aware explanations, and right-clicking a paper sentence opens a Chinese grammar study workflow. Native text selection now remains visibly selected across Reader re-renders until the user clicks elsewhere. Existing libraries, settings, prompts, model assignments, conversations, translations, and Stronghold credentials remain compatible across the upgrade."
