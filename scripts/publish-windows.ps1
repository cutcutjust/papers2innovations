...
$Manifest = [ordered]@{
  version = $Version
  notes = "v0.1.36 fixes numbered LaTeX equations that were displayed as red source text. Reader now promotes historical single-line double-dollar equations to true display math before KaTeX runs, while the visual PDF pipeline writes all new numbered equations with standards-compliant multiline delimiters. Existing Markdown is repaired at render time without reprocessing or additional model calls. Libraries, model roles, prompts, Stronghold credentials, translations, conversations, and regional vision caches remain compatible across the upgrade."
