...
$Manifest = [ordered]@{
  version = $Version
  notes = "Fixes Reader zoom so body text, links, tables, and formulas scale together. Translation now exposes missing credentials and empty states, supports one-click section translation, replaces translated text in place, and saves results automatically. Provider keys recover across model-ID and settings migrations through Stronghold plus an operating-system credential backup. Citation Graph gains a deterministic concentric layout, search, depth and status filters, relationship highlighting, and a modern inspector. First-run setup now creates a recommended local library, while Zotero supports automatic discovery or a manually selected data directory. Upgrades preserve libraries, settings, prompts, model assignments, and credentials."
