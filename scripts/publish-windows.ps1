...
$Manifest = [ordered]@{
  version = $Version
  notes = "Model configuration now refreshes reliably across the native application. Visual and OCR runtime roles are synchronized through ordered queues, Stronghold access is serialized, and PDF import waits for the selected vision model before checking readiness or starting work. This removes the race where a delayed clear operation could overwrite a newly configured model. The import dialog distinguishes an unassigned vision role from a missing credential, offers one-click reuse of an existing model, supports explicit retry, and revalidates immediately before copying files. Existing libraries, model settings, prompts, credentials, translations, and conversations remain compatible across the upgrade."
