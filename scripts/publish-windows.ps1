...
$Manifest = [ordered]@{
  version = $Version
  notes = "Introduces Immersive Reading with persistent 80%-180% zoom, Ctrl+wheel scaling, eye-comfort themes, sentence-aligned bilingual overlays, terminology popovers, and separate translation/chat annotations. Reader inline questions and the right assistant now share one persistent per-paper conversation and editable paper context, isolated from multi-paper research context. Adds independently assigned OpenAI/Anthropic vision analysis for cached figure explanations and confidence-gated formula repair. Migration 0013 preserves existing context, translations, settings, encrypted credentials, papers, and prompts during upgrade."
