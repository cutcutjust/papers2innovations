...
$Manifest = [ordered]@{
  version = $Version
  notes = "PDF imports now preview page counts and estimated model calls before confirmation, then use the configured vision model to reconstruct every page as structured Markdown with Docling coordinates, page caching, header and footer cleanup, formula checks, figures, tables, and explicit PDF verification for uncertain content. Reader-side Markdown formatting has been removed because document cleanup now belongs to the resumable import pipeline. Translation uses a dedicated model role, smaller sentence batches, adaptive output budgets, direct-output controls where supported, and preserves successfully translated sentences when a response is incomplete. Existing libraries, prompts, translations, conversations, model settings, and Stronghold credentials remain compatible across the upgrade."
