...
$Manifest = [ordered]@{
  version = $Version
  notes = "PDF-to-Markdown import now uses the configured vision model through the correct restricted cache path instead of silently falling back before transmission. Page-sized background rasters are rejected as figures; real figures and tables are cropped from validated local regions. Tables receive dedicated cell-by-cell visual reconstruction with Markdown and CSV artifacts, suspicious formulas are repaired from high-resolution bbox crops, references are separated into readable entries, and structural quality failures trigger visual verification. Reader tables now scroll cleanly with distinct headers and stable columns. Existing libraries, models, prompts, credentials, translations, and conversations remain compatible across the upgrade."
