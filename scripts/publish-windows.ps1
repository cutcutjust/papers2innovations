...
$Manifest = [ordered]@{
  version = $Version
  notes = "Fixes paper-file dragging inside the Windows desktop collection tree by preserving pointer capture across React renders, avoiding selection changes at drag start, and adding a visible paper drag handle. Folder dragging and central-list paper grouping remain supported. Includes the new category-based prompt library for Reader chat, translation, explanation, Markdown cleanup, and Innovation. Existing libraries, settings, encrypted credentials, papers, and prompt data remain preserved during upgrade."
