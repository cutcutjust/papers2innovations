...
$Manifest = [ordered]@{
  version = $Version
  notes = "Adds per-agent reusable prompt templates with create, read, update, delete, and one-click run workflows. Prompt templates are stored in the local paper library, remain isolated between agents, and existing agents receive a default Chinese analysis template. The agent run editor is now more compact, while existing libraries, settings, and encrypted credentials remain preserved during upgrade."
