...
$Manifest = [ordered]@{
  version = $Version
  notes = "The new local-first setup guides first-time users through creating a library, assigning text and vision models, and adding local PDFs. Add Papers now combines the native file picker and Windows drag-and-drop with SHA-256 verification and atomic copying, while Zotero remains available as an optional source in the same panel. The model page shows clear text and vision readiness, supports one multimodal model for both roles, and never enables OCR consent automatically. Existing libraries, settings, prompts, model assignments, conversations, translations, and Stronghold credentials remain compatible across the upgrade."
