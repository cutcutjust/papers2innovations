...
$Manifest = [ordered]@{
  version = $Version
  notes = "Replaces the agent execution center with a category-based local prompt library for Reader chat, translation, explanation, Markdown cleanup, and the Innovation workspace. Adds reliable pointer-driven paper and folder grouping for the Windows desktop WebView, with exact drop targeting and persistent collection updates. Existing libraries, settings, encrypted credentials, papers, and legacy prompt data remain preserved during upgrade."
