...
> 当前版本：Windows x64 `v0.1.27`。项目仍处于 `0.1.x` 快速迭代阶段，请保留不可替代源 PDF 的备份。
...
首次打开会引导创建本地论文库、配置文本/视觉模型并添加本地 PDF。本地导入始终是主路径；Zotero 位于“添加论文”的可选导入方式中。Zotero 会从 profile 自动发现；发现失败时可手动选择包含 `zotero.sqlite` 的数据目录。导入前可筛选 collection、预览页数和文件，PDF 校验 SHA-256 后原子复制，P2I 不修改 Zotero 数据。
...
2. 按首次引导创建推荐论文库，或选择一个独立的已有论文库目录。
3. 配置至少一个文本模型和一个视觉模型；支持用同一个多模态模型承担两种角色，也可以稍后配置。
4. 点击“添加论文”选择或拖入本地 PDF；需要时在同一面板打开可选的 Zotero 导入向导。
5. 在“任务活动”查看哈希、版面、OCR、图片、表格和索引阶段；失败后从对应阶段重试。
