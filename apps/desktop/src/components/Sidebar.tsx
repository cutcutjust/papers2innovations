...
export function Sidebar({ root, papers, collections }: { root: string; papers: LibraryPaper[]; collections: LibraryCollection[] }) {
  ...
  const renderNode = (node: CollectionTreeNode, depth: number): ReactNode => {
    ...
    return <div className="collection-tree-node" key={node.id}>
      ...
      {isExpanded && <div className="collection-tree-children">{node.children.map((child) => renderNode(child, depth + 1))}{directPapers.map((paper) => <button key={paper.id} className={`collection-paper-leaf ${selectedPaperId === paper.id ? "selected" : ""}`} style={{ paddingLeft: `${32 + (depth + 1) * 14}px` }} title={`${paper.title}\n单击选中，拖动到其他分类`} onClick={() => { selectPaper(paper.id); setView("library"); }} onDoubleClick={() => openReader(paper.id)} onPointerDown={(event) => startPointerCollectionDrag({ kind: "paper", id: paper.id }, event)}><FileText size={13} /><span>{paper.title}</span><i className="collection-paper-drag-handle" title="拖动论文归组"><GripVertical size={12} /></i></button>)}{renderEditor(node.id)}</div>}
    </div>;
  };
