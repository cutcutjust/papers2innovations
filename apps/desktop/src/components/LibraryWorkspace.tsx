...
import { BookOpen, BookOpenText, CalendarClock, CheckCircle2, ChevronRight, Clock3, FileText, Filter, FolderOpen, Grid2X2, GripVertical, Layers3, List, RefreshCw, Search, SortAsc, Star, Upload } from "lucide-react";
...
export function LibraryWorkspace({ papers, allPapers, collections, selected, scope, favoriteBusyId, scanning, onScan, onChooseLibrary, onToggleFavorite, onShowAll }: Props) {
        ...
        <aside className="library-inspector">
          {!selected ? <div className="empty-inspector"><BookOpen size={30} /><span>选择一篇论文查看详情</span></div> : <>
            ...
            <div className="inspector-content">
              ...
              <div className="inspector-actions"><button className="primary-button compact" onClick={() => openReader(selected.id)}><BookOpen size={13} /> {selected.lastReadAt ? "继续阅读" : "开始阅读"}</button><button className="secondary-button" onClick={() => useWorkspace.getState().setView("context")}><Layers3 size={13} /> 添加到上下文</button><button className="secondary-button"><CheckCircle2 size={13} /> 导出引用 <ChevronRight size={12} /></button></div>
            </div>
          </>}
        </aside>
      </div>}
