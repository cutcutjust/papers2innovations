...
import { AlertTriangle, CheckCircle2, Database, FileInput, FolderOpen, ListFilter, LoaderCircle, RefreshCw } from "lucide-react";
import { chooseZoteroDirectory, getOcrStatus, importFromZotero, inspectZotero, previewZoteroImport } from "../lib/bridge";

const ZOTERO_DIRECTORY_KEY = "p2i.zoteroDataDir";

export function ZoteroImport({ root, onChooseLibrary }: { root: string; onChooseLibrary: () => Promise<void> }) {
  const queryClient = useQueryClient();
  const [sourceDirectory, setSourceDirectory] = useState(() => window.localStorage.getItem(ZOTERO_DIRECTORY_KEY) ?? "");
  const inspection = useQuery({ queryKey: ["zotero-inspection", sourceDirectory], queryFn: () => inspectZotero(sourceDirectory || undefined), retry: false });
  const ocrStatus = useQuery({ queryKey: ["ocr-status"], queryFn: getOcrStatus, retry: false });
  const preview = useQuery({ queryKey: ["zotero-preview", inspection.data?.dataDir], queryFn: () => previewZoteroImport(inspection.data?.dataDir), enabled: Boolean(inspection.data && !inspection.data.locked), retry: false });
  ...
  const chooseSourceDirectory = async () => {
    const selected = await chooseZoteroDirectory();
    if (!selected) return;
    window.localStorage.setItem(ZOTERO_DIRECTORY_KEY, selected);
    setSelection({});
    setSourceDirectory(selected);
  };
  const useAutomaticDiscovery = () => {
    window.localStorage.removeItem(ZOTERO_DIRECTORY_KEY);
    setSelection({});
    setSourceDirectory("");
  };
  ...
  return <main className="import-page">
    ...
    {inspection.isLoading ? <div className="inline-loading"><LoaderCircle className="spin" /> 正在自动发现 Zotero 数据目录...</div> : inspection.isError ?
      <div className="zotero-onboarding"><div className="zotero-onboarding-icon"><Database size={24} /></div><div><h2>未能打开 Zotero 数据库</h2><p>系统会优先读取 Zotero profile 中的 <code>dataDir</code>。如果你使用便携版或自定义目录，请选择包含 <code>zotero.sqlite</code> 和 <code>storage/</code> 的文件夹。</p><small>{inspection.error instanceof Error ? inspection.error.message : String(inspection.error ?? "Zotero 检查失败")}</small><div><button className="primary-button compact" onClick={() => void chooseSourceDirectory()}><FolderOpen size={14} /> 选择 Zotero 数据目录</button><button className="secondary-button" onClick={() => void inspection.refetch()}><RefreshCw size={14} /> 重新自动发现</button></div></div></div> : inspection.data?.locked ?
      <div className="notice error-notice"><AlertTriangle size={19} /><div><strong>Zotero 论文库已锁定</strong><p>{inspection.data.lockReason}</p><p>请正常关闭 Zotero 后刷新。正式导入不会使用备份数据库。</p></div></div> : inspection.data && <>
        <section className="source-band zotero-source-band"><Database size={18} /><div><strong>{inspection.data.dataDir}</strong><span>{inspection.data.itemCount} 个条目 | {inspection.data.pdfCount} 个 PDF | {inspection.data.missingPdfCount} 个缺失</span></div><span className="source-ready"><CheckCircle2 size={14} /> 只读</span><button className="secondary-button" onClick={() => void chooseSourceDirectory()}><FolderOpen size={13} /> 更换目录</button>{sourceDirectory && <button className="text-button" onClick={useAutomaticDiscovery}>恢复自动发现</button>}</section>
        ...
        <footer className="import-footer"><div><strong>目标论文库</strong><span>{root}\Papers\Zotero</span><button className="text-button" onClick={() => void onChooseLibrary()}>更换位置</button></div><button className="primary-button" disabled={!root || !selected.length || importMutation.isPending} onClick={() => importMutation.mutate()}>{importMutation.isPending ? <LoaderCircle className="spin" size={16} /> : <FileInput size={16} />} 导入 {selected.length} 篇论文</button></footer>
